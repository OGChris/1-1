...
<script type='text/javascript'>
  /* eslint-disable no-plusplus,max-len,no-unused-vars,no-param-reassign,prefer-const */
  ...
  export default {
    ...
    props: {
      ...
      /*
       * @param viewFormat
       *    Expects an array of 3 values
       *    Value 1 is the moment format string i.e 'YYYY-MM-DD HH:mm:ss'
       *    Value 2
       */
      viewFormat: {
        type: Array,
    ...
    watch: {
      value(val, oldVal) {
        if (val !== oldVal) {
          this.formattedValue = this.momentFilter(moment(val), this.viewFormat[0] || false, this.viewFormat[1] || false, this.viewFormat[2] || false);
        }
      },
      minDate(val, oldVal) {
        if (this.control) {
